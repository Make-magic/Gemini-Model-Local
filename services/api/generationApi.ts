...
export const generateImagesApi = async (apiKey: string, modelId: string, prompt: string, aspectRatio: string, abortSignal: AbortSignal, baseUrl?: string): Promise<string[]> => {
    logService.info(`Generating image with model ${modelId}`, { prompt, aspectRatio, proxy: baseUrl });
...
export const generateSpeechApi = async (apiKey: string, modelId: string, text: string, voice: string, abortSignal: AbortSignal, baseUrl?: string): Promise<string> => {
    logService.info(`Generating speech with model ${modelId}`, { textLength: text.length, voice, proxy: baseUrl });
...
export const transcribeAudioApi = async (apiKey: string, audioFile: File, modelId: string, options: { isThinkingEnabled: boolean }, baseUrl?: string): Promise<string> => {
    const { isThinkingEnabled } = options;
    logService.info(`Transcribing audio with model ${modelId}`, { fileName: audioFile.name, size: audioFile.size, thinking: isThinkingEnabled, proxy: baseUrl });
...
export const translateTextApi = async (apiKey: string, text: string, baseUrl?: string): Promise<string> => {
    logService.info(`Translating text... Proxy: ${baseUrl || 'None'}`);
...
export const generateSuggestionsApi = async (apiKey: string, userContent: string, modelContent: string, language: 'en' | 'zh', baseUrl?: string): Promise<string[]> => {
    logService.info(`Generating suggestions in ${language}... Proxy: ${baseUrl || 'None'}`);
...
export const generateTitleApi = async (apiKey: string, userContent: string, modelContent: string, language: 'en' | 'zh', baseUrl?: string): Promise<string> => {
    logService.info(`Generating title in ${language}... Proxy: ${baseUrl || 'None'}`);
