...
import { generateUniqueId, generateSessionTitle, pcmBase64ToWavUrl, showNotification, base64ToBlob, getActiveProxyUrl } from '../utils/appUtils';
...
export const useTtsImagenSender = ({
    ...
    const handleTtsImagenMessage = useCallback(async (
        ...
        const proxyUrl = getActiveProxyUrl(appSettings);

        try {
            if (isTtsModel) {
                const base64Pcm = await geminiServiceInstance.generateSpeech(keyToUse, currentChatSettings.modelId, text, currentChatSettings.ttsVoice, newAbortController.signal, proxyUrl);
                ...
                const imageBase64Array = appSettings.generateQuadImages
                    ? (await Promise.all([
                        geminiServiceInstance.generateImages(keyToUse, currentChatSettings.modelId, text, aspectRatio, newAbortController.signal, proxyUrl),
                        geminiServiceInstance.generateImages(keyToUse, currentChatSettings.modelId, text, aspectRatio, newAbortController.signal, proxyUrl),
                        geminiServiceInstance.generateImages(keyToUse, currentChatSettings.modelId, text, aspectRatio, newAbortController.signal, proxyUrl),
                        geminiServiceInstance.generateImages(keyToUse, currentChatSettings.modelId, text, aspectRatio, newAbortController.signal, proxyUrl),
                    ])).flat()
                    : await geminiServiceInstance.generateImages(keyToUse, currentChatSettings.modelId, text, aspectRatio, newAbortController.signal, proxyUrl);
