...
export const useChatClient = ({
    ...
    useEffect(() => {
        ...
        const initializeChat = async () => {
            ...
            // Resolve proxy settings directly from appSettings prop to be reactive and flag-aware
            let proxyUrl: string | undefined = undefined;
            if (
                appSettings.useCustomApiConfig && 
                appSettings.useApiProxy && 
                appSettings.apiProxyUrl && 
                appSettings.apiProxyUrl.trim().length > 0
            ) {
                proxyUrl = appSettings.apiProxyUrl.trim();
            }

            const ai = getApiClient(keyResult.key, proxyUrl);
            ...
            setChat(newChat);
            logService.info(`Chat object initialized/updated for session ${activeSessionId} with model ${modelIdToUse}. Proxy: ${proxyUrl || 'None'}`);
        };
