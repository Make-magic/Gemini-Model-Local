...
import { getKeyForRequest, getActiveProxyUrl } from '../utils/appUtils';
...
export const useFilePolling = ({
    ...
    useEffect(() => {
        const filesCurrentlyPolling = new Set<string>(pollingIntervals.current.keys());
        ...
        for (const fileId of filesThatShouldPoll) {
            if (!filesCurrentlyPolling.has(fileId)) {
                ...
                const poll = async () => {
                    ...
                    try {
                        const metadata = await geminiServiceInstance.getFileMetadata(keyResult.key, fileApiName, proxyUrl);
                ...
                pollingIntervals.current.set(fileId as string, intervalId);
