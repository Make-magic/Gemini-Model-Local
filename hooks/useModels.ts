...
import { getActiveApiConfig, getActiveProxyUrl, logService, sortModels } from '../utils/appUtils';
...
export const useModels = (appSettings: AppSettings) => {
    ...
    // Extract relevant settings for dependency array to avoid unnecessary re-fetches
    ...
    const fetchAndSetModels = useCallback(async () => {
        ...
        try {
            modelsFromApi = await geminiServiceInstance.getAvailableModels(apiKeysString, proxyUrl);
        } catch (error) {
    ...
    }, [appSettings, modelsLoadingError]);
