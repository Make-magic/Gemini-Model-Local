...
import { generateUniqueId, getKeyForRequest, fileToBlobUrl, getActiveProxyUrl } from '../utils/appUtils';
...
export const useFileUpload = ({
    ...
    const handleProcessAndAddFiles = useCallback(async (files: FileList | File[]) => {
        ...
        const uploadPromises = filesArray.map(async (file) => {
            ...
            if (shouldUploadFile) {
                ...
                try {
                    const uploadedFileInfo = await geminiServiceInstance.uploadFile(keyToUse, file, effectiveMimeType, file.name, controller.signal, proxyUrl);
    ...
    const handleAddFileById = useCallback(async (fileApiId: string) => {
        ...
        try {
            const fileMetadata = await geminiServiceInstance.getFileMetadata(keyToUse, fileApiId, proxyUrl);
            if (fileMetadata) {
