...
import { generateUniqueId, buildContentParts, createChatHistoryForApi, getKeyForRequest, generateSessionTitle, logService, getActiveProxyUrl } from '../utils/appUtils';
...
export const useMessageSender = (props: MessageSenderProps) => {
    ...
    const handleSendMessage = useCallback(async (overrideOptions?: { text?: string; files?: UploadedFile[]; editingId?: string }) => {
        ...
        if (!activeModelId) { 
            ...
            const newSession: SavedChatSession = { 
                ...
                settings: { 
                    ...
                    ...appSettings,
                    thinkingLevel: appSettings.thinkingLevel as 'LOW' | 'HIGH' | undefined
                }, 
                timestamp: Date.now() 
            };
        ...
        // --- Stateless Construction of History ---
        const historyMessages = effectiveEditingId 
            ? messages.slice(0, messages.findIndex(m => m.id === effectiveEditingId))
            : messages;
            
        // Filter history to exclude loading or error states if necessary, generally just ensure they are valid turns
        // Note: For a new session, messages is empty here initially because state update is async, 
        // but we handle the current turn via `promptParts` and append it to history in the API call implicitly via the SDK logic usually.
        // For stateless call, we must supply history + current turn separately.
        
        const historyForApi = await createChatHistoryForApi(historyMessages);
        // Append the current user message to the content payload for the API
        const fullContents = [...historyForApi, { role: 'user', parts: promptParts }];

        const apiProxyUrl = getActiveProxyUrl(appSettings);
        ...
        if (appSettings.isStreamingEnabled) {
            await geminiServiceInstance.sendChatStream(
                keyToUse,
                activeModelId,
                fullContents,
                config,
            ...
            await geminiServiceInstance.sendChatNonStream(
                keyToUse,
                activeModelId,
                fullContents,
                config,
        ...
        handleTtsImagenMessage, handleImageEditMessage
    ]);
