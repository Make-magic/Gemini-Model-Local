...
import { getKeyForRequest, logService, pcmBase64ToWavUrl, getActiveProxyUrl } from '../utils/appUtils';
...
export const useTextToSpeechHandler = ({
    ...
    const handleTextToSpeech = useCallback(async (messageId: string, text: string) => {
        ...
        try {
            const base64Pcm = await geminiServiceInstance.generateSpeech(key, modelId, text, voice, abortController.signal, proxyUrl);
