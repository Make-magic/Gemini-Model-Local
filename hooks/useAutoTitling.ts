...
import { getKeyForRequest, logService, generateSessionTitle, getActiveProxyUrl } from '../utils/appUtils';
...
export const useAutoTitling = ({
    ...
    const generateTitleForSession = useCallback(async (session: SavedChatSession) => {
        ...
        try {
            ...
            const newTitle = await geminiServiceInstance.generateTitle(keyResult.key, userContent, modelContent, language, proxyUrl);
